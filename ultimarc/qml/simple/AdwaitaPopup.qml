...
Item {
    ...
    Rectangle {
        ...
        anchors {
            fill: layout
            bottomMargin: -_units.gridUnit
            topMargin: anchors.bottomMargin - background.radius
            leftMargin: anchors.bottomMargin
            rightMargin: anchors.bottomMargin + _units.smallSpacing
        }
        border {
            color: "#3a3a3a"
            width: 1
    ...
    RowLayout {
        id: layout
        spacing:  _units.smallSpacing

        anchors.horizontalCenter: parent.horizontalCenter
        width: parent.width * 0.666
        y: root.open ? _units.gridUnit : -(height + _units.gridUnit + _units.smallSpacing)
