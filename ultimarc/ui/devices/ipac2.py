...
from ultimarc.devices.ipac2 import PinMapping, Ipac2Device
...
ipac2RoleMap = OrderedDict(zip(list(Ipac2Roles), [k.name.lower() for k in Ipac2Roles]))


class Ipac2UI(Device):
    ...
    def __init__(self, args, env, attached,
                 ...
                 key=''):
        super(Ipac2UI, self).__init__(args, env, attached, device_class_id,
                                      name, device_class_descr, key)

        self.icon = 'qrc:/logos/workstation'
        self.description = '32-Input button/joystick/trackball/spinner USB interface'
        self.config = None
        self._json_obj = None
        ...
        if self.config is None:
            if self.attached:
                devices = [dev for dev in
                           self.env.devices.filter(class_id=DeviceClassID.IPAC2, bus=self.args.bus,
                                                   address=self.args.address)]
                for dev in devices:
                    with dev as dev_h:
                        self.config = dev_h.to_json_str(dev_h.read_device())
            else:
                self.config = {'schemaVersion': 2.0, 'resourceType': 'ipac2-pins',
    ...
    def write_file(self, file):
        if Ipac2Device.validate_config(self.config, 'ipac2.schema'):
            Ipac2Device.write_to_file(self.config, file.toLocalFile(), indent=2)
            return True
        return False

    def load_file(self, file):
        resource_types = ['ipac2-pins']
        config = Ipac2Device.validate_config_base(file.toLocalFile(), resource_types)

        if config is not None:
            if Ipac2Device.validate_config(config, 'ipac2.schema'):
                self.config = config
                self._json_obj = JSONObject(self.config)
                self.set_macros()
    ...
    def data(self, index: QModelIndex, role: int = ...) -> typing.Any:
        ...
        if role == Ipac2Roles.NAME:
            return pin_name

        for pin in self._json_obj.pins:
            if pin.name == pin_name:
                if role == Ipac2Roles.ACTION:
                    return pin.action
                if role == Ipac2Roles.ALT_ACTION:
                    try:
                        return pin.alternate_action
                    except AttributeError:
                        return ''
                if role == Ipac2Roles.SHIFT:
                    try:
                        return pin.shift
                    except AttributeError:
                        return False
        else:
            return False if role == Ipac2Roles.SHIFT else ''
